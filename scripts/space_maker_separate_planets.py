...
def make_planet_splats_separate(
    n_planets: int,
    radius: float,
    seed: int,
    planet_radius_min: float,
    ...
    """
    행성을 scene 전체와 합치지 않고, 행성별 splat 묶음으로 따로 반환한다.

    반환값:
        planet_entries: list[dict]
        각 dict는 index, center, radius, points, colors, opacities, scales, rotations를 가진다.

    이 함수는 make_planet_splats()와 같은 생성 규칙을 사용하지만,
    마지막에 np.concatenate로 모든 행성을 하나로 합치지 않는다.
    따라서 planet1.ply, planet2.ply처럼 행성별 PLY 저장에 사용할 수 있다.
    """
    rng = np.random.default_rng(seed + 12000)
    ...
    # 기존 make_planet_splats()와 동일한 rejection sampling 배치.
    for i in range(n_planets):
        pr = float(rng.uniform(planet_radius_min, planet_radius_max))
        for _ in range(200):
            direction = _random_unit_vector(rng)
    ...
    for i, (center, pr) in enumerate(zip(centers, planet_radii)):
        ...
        one_points.append(points.astype(np.float32))
        one_colors.append(colors.astype(np.float32))
        one_opacities.append(surf_opacities)
        one_scales.append(surf_scales)
        one_rotations.append(surf_rotations)
        ...
        if make_ring:
            ...
            one_points.append(ring_points.astype(np.float32))
            one_colors.append(ring_colors.astype(np.float32))
            one_opacities.append(ring_opacities)
            one_scales.append(ring_scales)
            one_rotations.append(ring_rotations)
        ...
        planet_entries.append(
            {
                ...
                "radius": float(pr),
                "points": np.concatenate(one_points, axis=0).astype(np.float32),
                "colors": np.concatenate(one_colors, axis=0).astype(np.float32),
                "opacities": np.concatenate(one_opacities, axis=0).astype(np.float32),
                "scales": np.concatenate(one_scales, axis=0).astype(np.float32),
                "rotations": np.concatenate(one_rotations, axis=0).astype(np.float32),
            }
        )
...
def main():
    ...
    parser.add_argument(
        "--separate-planets",
        action="store_true",
        help="켜면 행성을 main scene PLY에 합치지 않고 planet1.ply, planet2.ply처럼 별도 저장한다.",
    )
    parser.add_argument(
        "--planet-output-dir",
        type=str,
        default=None,
        help="별도 행성 PLY 저장 폴더. 기본값은 --output 파일이 있는 폴더이다.",
    )
    parser.add_argument(
        "--planet-prefix",
        type=str,
        default="planet",
        help="별도 저장할 행성 PLY 파일 접두어. 예: planet -> planet1.ply",
    )
    ...
    planet_splat_count = 0

    if args.separate_planets:
        print("Making procedural planets as separate PLY files")
        planet_entries = make_planet_splats_separate(
        ...
        points = np.concatenate([dust_points, star_points, inner_star_points], axis=0)
        colors = np.concatenate([dust_colors, star_colors, inner_star_colors], axis=0)
        opacities = np.concatenate([dust_opacities, star_opacities, inner_star_opacities], axis=0)
        scales = np.concatenate([dust_scales, star_scales, inner_star_scales], axis=0)
        rotations = np.concatenate([dust_rotations, star_rotations, inner_star_rotations], axis=0)

        output_dir = os.path.dirname(args.output)
        if args.planet_output_dir is not None:
            planet_output_dir = args.planet_output_dir
        elif output_dir:
            planet_output_dir = output_dir
        else:
            planet_output_dir = "."
        os.makedirs(planet_output_dir, exist_ok=True)

        for planet in planet_entries:
            planet_path = os.path.join(
                planet_output_dir,
                f"{args.planet_prefix}{planet['index']}.ply",
            )
            planet_splat_count += planet["points"].shape[0]
            print(
                f"Writing {planet_path} "
                f"({planet['points'].shape[0]} splats, "
                f"center={planet['center']}, radius={planet['radius']:.4f})"
            )
            write_3dgs_ply_binary_full(
                path=planet_path,
                points=planet["points"],
                colors=planet["colors"],
                opacities=planet["opacities"],
                scales=planet["scales"],
        ...
        planet_splat_count = planet_points.shape[0]
    ...
    print(f"Planet/ring splats: {planet_splat_count}")
    print(f"Total splats in main scene: {points.shape[0]}")
